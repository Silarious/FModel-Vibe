...
Bulk folder exports and Export Queue runs use dedicated `LongRunning` workers (not the shared ThreadPool for the heavy path). Parallelism is controlled under **Settings → General → MULTI THREADING & LIMITS**.

### Settings

| Setting | What it does |
| --- | --- |
| **Use Concurrent Workers** | On: use the worker counts below. Off: single worker (DOP = 1). |
| **Export Max Concurrent Workers** | Base degree of parallelism for folder / queue exports. `0` = auto (usable cores = CPU count − Reserved CPU Threads). |
| **Add Workers for Image/Models** | Extra workers (+0…+16) added to the base DOP when exporting textures/images or models (including All Assets). Still capped at usable cores. |
| **Large File Workers (>10MB)** | Cap concurrent workers for packages &gt;10MB and ≤100MB (default 2). Still capped by Export Max Concurrent Workers. |
| **Huge File Workers (>100MB)** | Cap concurrent workers for packages &gt;100MB (default 1). |
| **Reserved CPU Threads** | Logical CPUs excluded from FModel via process affinity (0–4, default 2). Keeps cores free for the OS/other apps. Distinct from worker count. |
| **Enable Memory Guard** | Soft/hard RAM limits: throttle or stop issuing new export work so the process cannot soft-lock the machine. Does not kill FModel. |
| **Memory Soft Target %** | Soft threshold as % of physical RAM (default 55). Above it, workers pause taking new packages until memory drops. |
| **Memory Hard Cap %** | Hard threshold (default 85). Stop new work; allow at most one in-flight package while recovering. |
| **Export Perf Log** | Write `Logs/FModel-ExportPerf-*.log` for bulk/queue runs (slow exports by ms/MB, large-file section, memory/GC deltas). |
| **Perf Log Min Duration (ms)** | Only log exports that take at least this long (default 250). |

Related export options (Export Behavior section): **Export Smallest Files First**, **Group Export Loose Assets**.
...
- Collapsible **SeparatorExpander** sections on General (Advanced, Export Behavior, **MULTI THREADING & LIMITS**, Fortnite LIVE) with expanded state persisted per profile
