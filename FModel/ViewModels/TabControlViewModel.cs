...
public class TabImage : ViewModel
{
    ...
    private void SetImage(SKBitmap bitmap)
    {
        ...
        using var data = _bmp.EncodeSafe(NoAlpha ? SKEncodedImageFormat.Jpeg : SKEncodedImageFormat.Png, 100);
    ...
    private void SetImage(CTexture bitmap)
    {
        ...
        byte[] imageData;
        lock (TextureEncoder.SkiaEncodeSync)
        {
    ...
    public static bool TryEncodeForExport(string name, SKBitmap bitmap, bool noAlpha, out string exportName, out byte[] buffer)
    {
        ...
        try
        {
            using var data = bitmap.EncodeSafe(noAlpha ? SKEncodedImageFormat.Jpeg : SKEncodedImageFormat.Png, 100);
            buffer = data.ToArray();
            exportName = $"{name}.{(noAlpha ? "jpg" : "png")}";
            return buffer is { Length: > 0 };
        }
